    ...
    public String execute(Application application, User user) {
        ...
        if(res.size()!=0){
            StringBuilder builder = new StringBuilder(
                    UIApp.localeClass.getString("dragons_with_names_start_with.text")
                            + args[0] + ": " + res.size() +"\n");
            res.stream().map(Dragon::getName).forEach(d ->builder.append(d).append("\n"));
            return builder.toString();
        }else{
            return UIApp.localeClass.getString("dragons_with_names_start_with.text") + args[0]
                    + UIApp.localeClass.getString("no.text");
        }
    }
    @Override
